...
# The modules a member's error passes through, hashed into the artifact for the
# same reason the serving column hashes its own: a derived file that outlives
# the code producing it reads as current and is not. Whole-file on purpose - it
# can only over-fire, and over-firing costs a 7-minute re-run and says so.
CODE_PATHS = (
    ROOT / "kernelverify" / "schemas" / "quant_contract.py",
    ROOT / "bench" / "calibrate_quant_device.py",
