"""LLM-style buggy LeakyReLU: alpha = 0.1 instead of 0.01 (10x too leaky).

A common transcription error - PyTorch's default is 0.01, some other frameworks
use 0.1, and LLMs often confuse them. Outputs differ on negative inputs only."""
...
WRONG_ALPHA = 0.1  # BUG: should be 0.01


if _TRITON_OK:

    @triton.jit
    def _leaky_relu_buggy_kernel(out_ptr, in_ptr, n_elements,
...
def run(inputs):
    if not _TRITON_OK:
        raise RuntimeError("leaky_relu_triton_buggy requires torch + triton + CUDA")
    ...
    _leaky_relu_buggy_kernel[grid](y_t, x_t, n, BLOCK=BLOCK, ALPHA=WRONG_ALPHA)
