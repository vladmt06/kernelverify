"""Correct Triton RMSNorm: y = x * rsqrt(mean(x^2) + eps), reduce over last axis."""
...
if _TRITON_OK:
    ...
    @triton.jit
    def _rmsnorm_kernel(out_ptr, in_ptr, in_row_stride, out_row_stride, n_cols,
        ...
        # rsqrt via 1/sqrt; tl.sqrt is universally available.
...
def run(inputs):
    if not _TRITON_OK:
        raise RuntimeError("rmsnorm_triton requires torch + triton + CUDA")
    x = inputs["input"]
    dt = x.dtype
    orig_shape = x.shape
    ...
    _rmsnorm_kernel[(n_rows,)](y_t, x_t, x_t.stride(0), y_t.stride(0), n_cols,
                               BLOCK=BLOCK, EPS=EPS)
