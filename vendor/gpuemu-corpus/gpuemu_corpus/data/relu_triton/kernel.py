"""Correct Triton ReLU: y = max(0, x). Elementwise."""
...
if _TRITON_OK:
    ...
    @triton.jit
    def _relu_kernel(out_ptr, in_ptr, n_elements, BLOCK: tl.constexpr):
        pid = tl.program_id(0)
...
def run(inputs):
    if not _TRITON_OK:
        raise RuntimeError("relu_triton requires torch + triton + CUDA")
    ...
    _relu_kernel[grid](y_t, x_t, n, BLOCK=BLOCK)
