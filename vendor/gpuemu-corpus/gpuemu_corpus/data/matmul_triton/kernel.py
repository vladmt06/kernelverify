"""Correct Triton matmul: C[M,N] = A[M,K] @ B[K,N].

Block-based with per-element accumulation (no tl.dot — works for any M/N/K
including the small/odd shapes the schema fuzzes). fp32 accumulator regardless
of input dtype, cast back to input dtype on store.
"""
...
def run(inputs):
    if not _TRITON_OK:
        raise RuntimeError("matmul_triton requires torch + triton + CUDA")
