"""Correct Triton L2-norm over the last axis: y = x / sqrt(sum(x^2) + eps)."""
...
if _TRITON_OK:
    ...
    @triton.jit
    def _l2norm_kernel(out_ptr, in_ptr, in_row_stride, out_row_stride, n_cols,
...
def run(inputs):
    if not _TRITON_OK:
        raise RuntimeError("l2norm_triton requires torch + triton + CUDA")
    ...
    _l2norm_kernel[(n_rows,)](y_t, x_t, x_t.stride(0), y_t.stride(0), n_cols,
                              BLOCK=BLOCK, EPS=EPS)
