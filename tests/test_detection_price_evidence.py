...
def test_the_prerepair_records_still_name_the_code_that_produced_them():
    """The staleness guard on the "before" half.

    Both record sets are pre-merge artifacts, so a test that only compares them
    to each other stays green while the live code drifts away from both - the
    two files agree with each other for ever. This compares the committed header
    against the LIVE code identity instead, so a merge or an edit that moves the
    member, the error metric or the device harness fails here and is re-run.
    """
    from derive_prerepair_device_records import code_identity

    header = json.loads(BEFORE.read_text())["derived_from"]
    assert header["after_sha256"] == hashlib.sha256(AFTER.read_bytes()).hexdigest(), (
        "the post-repair records moved; the comparison is no longer against them")
    assert header["code_sha256"] == code_identity(), (
        "the pre-repair records predate the code that produces them; re-run\n"
        "  .venv/bin/python -u bench/derive_prerepair_device_records.py")
    assert header["member_swapped"] == REPAIRED
