...
- The Day 1 stock training profile, which selects the sprint's first operation. Four modules, split so the arithmetic can be read and argued with on a machine with no GPU and the harness cannot quietly become the thing that decides.
  `bench/profile_rules.py` is the decision arithmetic and measures nothing: the gain formula, the credited ratio and its per-direction collapse, the reconciliation, the compile transfer, the instrument cost, the kill rule and the selection, each naming the pre-registration section that fixed it.
  ...
  `bench/profile_stock.py` is the harness: one child per cell, one batch pulled from mlx-lm's own iterator, every mode driven in rotation, and a recording that refuses to bind unless the region counts match the arrangement, the marked pass computes what the plain pass computes, and the instrument's cost sits inside a band somebody registered in advance.
  `bench/pin_dolly.py` freezes the corpus slice and measures the distribution a band is chosen against; mlx-lm pads each batch only to its own longest row, so the width a step runs at is a property of the data and `max_seq_length` is a cap rather than a target.
  ...
  `bench/profile_instrument.py` is demoted, not deleted. Its timings are wrong and its COUNTS are exact, and those counts have caught a seam that never fired and a test that left gradient checkpointing installed for a whole session. It stays as a structural check on call counts and gradient identity, run once per cell and per width, and no time is ever read from it.
  One fault in committed code remains open: `interleave.interleaved_samples` takes exactly two arms, which a four-point dial ladder cannot use. The other one is fixed: `select_first_operation` used to test the shipping floor only against the largest gain and could return a below-floor candidate through the tie-breaks, reproduced by execution at 1.0812 against 1.10; since 2026-08-20 the tie band is drawn only from candidates at or above the floor per Amendment 5 clause 24, and below-floor candidates stay in the ranked evidence but never in `selected` or `tied_with`.
  ...
  Candidate A has three built dials and no chosen one, deliberately. `ATTENTION_KNOBS` holds all three and `KNOBS` has no entry for "A", because clause 15's criterion returns a different dial at each width and clause 15 registers no width: at 96 queries the length dial wins and at 384 queries a head-dimension dial does. Do not add `KNOBS["A"]` before that is ruled; the dial sets candidate A's share and the two answers differ by a factor of two and a half.
