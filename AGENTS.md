...
- The Day 1 stock training profile, which selects the sprint's first operation. Four modules, split so the arithmetic can be read and argued with on a machine with no GPU and the harness cannot quietly become the thing that decides.
  ...
  Two things are unresolved, both written up in TODOS.md: how section 3.3 should attribute a share now that its registered method is known wrong, and the fact that the corpus cannot produce the sequence length the pre-registration names.
