...
- The Day 1 stock training profile, which selects the sprint's first operation. Four modules, split so the arithmetic can be read and argued with on a machine with no GPU and the harness cannot quietly become the thing that decides.
  ...
  Its manifest is Amendment 8's, 26 arms at the short width and 35 at the long: stock 1, candidate L 9, candidate Q 8, candidate P3 8, and candidate A's 9 at the long width alone.
  ...
  `--decide` refuses a schema-2 recording outright until `profile_rules.py` is rewritten to Amendment 7's scope, because the old arithmetic over the new recording would produce a ruling with the right shape and the wrong meaning.
