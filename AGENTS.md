...
- The Day 1 stock training profile, which selects the sprint's first operation. Four modules, split so the arithmetic can be read and argued with on a machine with no GPU and the harness cannot quietly become the thing that decides.
  ...
  Two things are measured and unresolved, both written up in TODOS.md: every share carries its own marks, so it is an upper bound whose overstatement grows with mark count, and the corpus cannot produce the sequence length the pre-registration names.
