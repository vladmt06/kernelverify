...
- The Day 1 stock training profile, which selects the sprint's first operation. Four modules, split so the arithmetic can be read and argued with on a machine with no GPU and the harness cannot quietly become the thing that decides.
  ...
  Two faults in committed code that Amendment 5 makes fixable and that are worth knowing about on their own: `select_first_operation` tests the shipping floor against the largest gain and then picks from inside the tie band, so it can return a candidate below the floor, reproduced by execution at 1.0812 against 1.10; and `interleave.interleaved_samples` takes exactly two arms, which a four-point dial ladder cannot use.
