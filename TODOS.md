...
## ADR 0016's detection-price "before" column has no committed evidence

- What: re-run `bench/calibrate_quant_device.py` on a GPU with `member_factored_groups` reverted to its pre-repair form (`0052f7b`), over the same rng stream, and commit the resulting records so ADR 0016's "ratio before" column (1.248 / 1.640 / 1.963) and its 39 / 96 / 96 / 96 boundary counts can be recomputed rather than remembered.
- Why: this lane's own rule is that an ADR's evidence gets preserved, and the serving half now satisfies it (`bench/results/quant_serving_repaired_member.json`, recomputed by test on every run); the device half's "before" numbers came from a verbatim pre-repair re-run whose records were never committed, so they rest on a run that no longer exists.
- Pros: closes the last asymmetry the merge review found, and the harness and revert point are both already identified, so the work is a run rather than a design.
- Cons: needs the Metal GPU and a temporary revert of a shipped member, so it cannot share a window with any other measurement; ~7 minutes of run inside a larger setup.
- Context: the "after" halves already reproduce from the committed `bench/results/quant_device_adequacy.json`; nothing in the K decision depends on the missing column, because K is decided by the serving demand (3.120) and the false-positive count (10 -> 0), both of which are backed.
- Depends on / blocked by: nothing; it should NOT be bundled with M1 (the ADR 0017 re-run under the G1 amendment), because mixing a reverted member into that reading would confound two changes in one measurement.
