...
class Claim:
    ...
    def unattested(self) -> bool:
        return self.row.get("provenance_tier") == UNATTESTED_TIER
...
def classify(row: dict) -> Claim:
    ...
    if not row.get("binding", False):
        blockers = row.get("binding_blockers") or ["binding is false with no blocker recorded"]
...
def utilisation(row: dict) -> tuple[str, float | None]:
    ...
    if roofline.get("binding_resource") == "compute":
...
def render_measurements(claims: list[Claim]) -> list[str]:
    rows = [c for c in claims
            if (c.row.get("measurement") or {}).get("kind") != "ceiling"]
    publishable = [c for c in rows if c.publishable and not c.unattested]
    if not publishable:
        return ["## Measurements", "",
                "No row in the record is publishable as an absolute number yet.",
                "Every measured row is listed under *Refused* below with the reason, "
...
def render_comparisons(claims: list[Claim]) -> list[str]:
    ...
    for claim in claims:
        ...
        if claim.unattested:
            continue
        if claim.group:
            groups[claim.group].append(claim)

    pairs = {name: members for name, members in groups.items() if len(members) > 1}
    if not pairs:
        return []

    lines = ["## Comparisons", "",
             "Only rows measured interleaved within one sampling group appear here. "
             "Rows from different groups are not paired even when both are binding, "
             "because an A/B run in separate passes measures the clock: one fixed "
             "shape drifted 131.7 to 93.1 us between runs minutes apart.", ""]
    for name, members in sorted(pairs.items()):
        members = sorted(members, key=lambda c: c.row.get("row_id", ""))
        # The baseline is the group's own publishable row where one exists, so
        ...
        lines.append("")
        lines.append(f"| Stack | Metric | Ratio vs {_stack_label(baseline.row)} | Note |")
        lines.append("|---|---|---|---|")
        for claim in members:
            ...
            if not ok:
                ...
                ratio = f"{value:.2f}x"
            lines.append(f"| {_stack_label(claim.row)} | {_fmt(median)} "
...
def render_refusals(claims: list[Claim]) -> list[str]:
    refused = [c for c in claims if not c.publishable and not c.unattested]
    if not refused:
        return []
    lines = ["## Refused", "",
...
def render_unattested(claims: list[Claim]) -> list[str]:
    unattested = [c for c in claims if c.unattested]
    if not unattested:
        return []
    lines = ["## Contributed claims (unattested)", "",
             "Rows from machines the project does not control. They are rendered "
             "separately and never merged into a binding result: a contributed row "
             "is a claim about someone else's machine until we can reproduce it.", "",
             "| Row | Stack | Kind | Value |", "|---|---|---|---|"]
...
def render(rows: list[dict]) -> str:
    ...
    published = sum(1 for c in claims if c.publishable and not c.unattested)
    header.append(
        f"{len(claims)} measured rows: {published} publishable, "
        f"{sum(1 for c in claims if not c.publishable and not c.unattested)} refused, "
        f"{sum(1 for c in claims if c.unattested)} contributed."
    )
    header.append("")
