...
t > TCAP is rejected loudly at every layer rather than left to the caller,
because overrunning a compile-time-sized threadgroup buffer is silent
memory corruption. Three complementary mechanisms: the kernel body poisons
the output with NaN and returns (a kernel cannot raise, and a poisoned
output fails every oracle and every generation instantly); the MLX door
raises before dispatch, where the cache length is visible host-side; and
`require_capacity()` is the same host-side check for anyone feeding
`kernel_spec()` cases directly through the raw door, the same obligation
class as SUPPORTED_BITS and SUPPORTED_DH. `should_dispatch` remains the
routing predicate; the rejections are the backstop for callers that skip it.
"""
...
def mlx_door_source() -> str:
    """The TU in the MLX door's spelling, the single source `build()` uses.

    The spelling follows the padded-cache door: the logical length arrives as
    the `t_cached` scalar input and the physical row stride and cache head
    count come from `k_wq`'s own shape. Extraction and certificates must
    capture THIS source, not the pre-integration one.
    """
